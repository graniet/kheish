...
| `execute_code` | One-shot scripts needing Kheish tool access (web_search, file ops). Stateless. |
...
This imported skill does not bundle the external `hamelnb` helper repository. Keep
the helper checkout separate from Kheish and point `SCRIPT` at that checkout:
```
