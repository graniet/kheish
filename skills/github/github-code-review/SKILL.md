...
[ "$AUTH" = "none" ] && echo "Run 'gh auth login' or export GITHUB_TOKEN before PR interactions" && exit 1
...
```bash
source .agents/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Kheish. Code looks clean — good test coverage, no security concerns."
