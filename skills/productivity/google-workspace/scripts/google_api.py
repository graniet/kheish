#!/usr/bin/env python3
"""Google Workspace API CLI for Kheish.

Thin wrapper that delegates to gws (googleworkspace/cli) via gws_bridge.py.
Maintains the same CLI interface for backward compatibility with Kheish skills.
...
def gws(*args: str) -> None:
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "KHEISH_HOME": os.environ.get("KHEISH_HOME", str(Path.home() / ".kheish"))},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Kheish (gws backend)")
