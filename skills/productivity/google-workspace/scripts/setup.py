#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Kheish.
...
def get_kheish_home() -> Path:
    """Return the profile-scoped home used by the Google Workspace helpers."""
    return Path(os.environ.get("KHEISH_HOME", Path.home() / ".kheish"))


def display_kheish_home() -> str:
    """Return a display-safe string for the Kheish profile home."""
    return str(get_kheish_home())


KHEISH_HOME = get_kheish_home()
TOKEN_PATH = KHEISH_HOME / "google_token.json"
CLIENT_SECRET_PATH = KHEISH_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KHEISH_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Kheish profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json into the Kheish profile home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kheish_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Kheish")
