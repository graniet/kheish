...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.kheish/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KHEISH_HOME", Path.home() / ".kheish")) / "skills" / "red-teaming" / "godmode"
...
KHEISH_HOME = Path(os.getenv("KHEISH_HOME", Path.home() / ".kheish"))
CONFIG_PATH = KHEISH_HOME / "config.yaml"
PREFILL_PATH = KHEISH_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous family models — already uncensored, just needs a clean prompt
    "hermes": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _get_current_model() -> tuple:
    """Read current model and provider from Kheish config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.kheish/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Kheish for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Kheish for changes to take effect.")
