...
This skill intentionally does not vendor a separate CLI implementation into Kheish. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.kheish/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.kheish/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.kheish/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
