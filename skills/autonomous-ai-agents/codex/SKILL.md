...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Kheish terminal. Codex is OpenAI's autonomous coding agent CLI.
...
5. **Background for long tasks** — use `background=true` and monitor with daemon task tools
6. **Don't interfere** — monitor with `task_list`/`task_output`, be patient with long-running tasks
