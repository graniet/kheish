...
**Activate `research:arxiv` with `use_skill`** for structured paper discovery when the skill is available in the current session. It provides arXiv REST API search, citation workflows, and BibTeX generation guidance.
...
## Reproduction
To reproduce Table 1: document the exact checked-in command or notebook path
To reproduce Figure 2: document the exact checked-in command or notebook path
...
This skill is designed for the Kheish agent. It uses Kheish tools, delegation, scheduling, and memory for the full research lifecycle.

### Related Skills

Compose this skill with other Kheish skills for specific phases:

| Skill | When to Use | How to Load |
|-------|-------------|-------------|
| **research:arxiv** | Phase 1 (Literature Review): searching arXiv, generating BibTeX, and finding related papers | `use_skill(name="research:arxiv")` |
| **software-development:subagent-driven-development** | Phase 5 (Drafting): parallel section writing with 2-stage review (spec compliance then quality) | `use_skill(name="software-development:subagent-driven-development")` |
| **software-development:plan** | Phase 0 (Setup): creating structured plans before execution | `use_skill(name="software-development:plan")` |
| **qmd** | Phase 1 (Literature): optional external local-search skill if your environment already provides it | external install, not bundled here |
| **creative:excalidraw** | Phase 4-5: creating Excalidraw-based figures and architecture diagrams | `use_skill(name="creative:excalidraw")` |
| **data-science:jupyter-live-kernel** | Phase 4 (Analysis): Jupyter live kernel for interactive analysis and visualization | `use_skill(name="data-science:jupyter-live-kernel")` |
...
| **`bash` + `task_list` / `task_output` / `task_stop`** | Background experiment management through daemon-owned shell tasks |
