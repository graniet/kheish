...
- `kheish-daemon shell_tasks::tests::persisted_target_validation_accepts_exec_replaced_leader` — process timing under load (spawns `exec sleep 30` and asserts on the replaced leader's /proc command); passes 3/3 in isolation. Same class as the pipe-holder flake.
- **Class note:** the daemon suite has a small population of load-sensitive tests; each loaded full run fails zero or one of them at random. Two members were fixed on 2026-07-03 with structural cures (SSE subscription race → Last-Event-ID cursor; health-check env race → hold `debug_capture_env_lock` around the `/v1/status` read, since the health snapshot reads `KHEISH_DEBUG_*` at request time). The remaining members need the same treatment: find the shared mutable state or timing assumption and remove it, don't retry-loop.
