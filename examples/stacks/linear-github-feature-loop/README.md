...
- the daemon is already started with Linear and GitHub MCP servers available; the Stack declares the required server and tool names under `spec.requires.mcp`, so `plan`, `apply`, and `verify` fail closed if the active daemon does not expose them;
