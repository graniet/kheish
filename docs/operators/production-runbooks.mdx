...
| Session storage growth | `/v1/status` `.storage.session_storage.oversized_session_count`, `.largest_session_bytes` | any oversized session (above 100 MiB) — plan a `sessions vacuum` window | oversized session still growing after a vacuum, or run starts visibly degrading |
