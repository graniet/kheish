...
Here are the three planes, side by side — three parallel stores that never merge into one another. Each plane flows down into its own storage, then into the two shared surfaces that read all three.

```mermaid
flowchart TD
    P1["Plane 1: Session journal<br/>the transcript"]
    P2["Plane 2: Recovered run memory<br/>episodic"]
    P3["Plane 3: Learning plane<br/>durable knowledge"]
    S1["Lives in the session journal and checkpoints<br/>canonical history"]
    S2["Lives in run-memories and a topology index<br/>derived read model"]
    S3["Lives in learnings, learning-candidates, learning-skills"]
    Prompt["Assembled into ONE prompt by the runtime<br/>each with its own bounded budget"]
    Inspect["Inspected together via GET /v1/sessions/ID/memory-context<br/>the what-is-eligible-now view"]
    P1 --> S1
    P2 --> S2
    P3 --> S3
    S1 --> Prompt
...
```mermaid
flowchart TD
    Terminal["A run reaches a TERMINAL status<br/>completed, failed, interrupted, or cancelled"]
    Build["BUILD RECORD<br/>request_preview from the durable payload<br/>outcome_preview, the latest assistant text<br/>failure_markers, status plus error preview<br/>artifact_ids<br/>summary is the joined parts, then capped at 480"]
    Scrub["SCRUB, before it ever touches disk<br/>secret and token redactor<br/>PII: email, phone, SSN, Luhn card, keys<br/>redaction_count fields are counted"]
    Persist["PERSIST and INDEX<br/>write to run-memories<br/>track a pointer in the topology index, by_session and by_scope<br/>prune on TTL plus per-session overflow"]
    NewInput["Later, a NEW input arrives on the SAME session"]
    Rank["RANK against the pending input<br/>lexical query-term scoring<br/>recency is the fallback and tie-breaker"]
    Budget["BUDGET and PACK<br/>keep at most max_prompt_entries, default 3<br/>drop older entries first if they do not fit<br/>if nothing fits, omit the section entirely"]
    Inject["INJECT as ONE system section named recovered_memory<br/>framed as historical run data, not orders<br/>code fences inside summaries are neutralized"]
    Terminal --> Build --> Scrub --> Persist --> NewInput --> Rank --> Budget --> Inject
```
...
```mermaid
flowchart TD
    Sess["1. SESSION<br/>scope key session:ID, always present, narrowest"]
    Pers["2. PERSONA<br/>scope key persona:ID, only if a persona is bound to the session"]
    Proj["3. PROJECTS<br/>scope key project:ID, one entry per project the session is linked to"]
    Work["4. WORKSPACE<br/>scope key workspace:default, always present, id is ALWAYS default, widest"]
    Sess --> Pers --> Proj --> Work
```

The scopes are resolved by `learning_scopes_for_session`. A learning is visible to the session if its scope key matches ANY row above: a `workspace:default` fact is visible to every session, while a `session:S` fact is visible only inside session S.
...
```mermaid
flowchart TD
    Capture["CAPTURE<br/>a completed run is scanned for durable memory<br/>heuristic labels Preference, Fact, Decision short-circuit the model at confidence 98<br/>else model-backed extraction of fact, preference, decision, bounded by max_candidates_per_run, default 2"]
    Candidate["CANDIDATE<br/>a LearningCandidateView is persisted with origin api or daemon, scope, kind, content, confidence, source, evidence_refs, sensitivity<br/>secret-looking content is rejected before storage"]
    Rules["RULES<br/>ordered publication rules, FIRST match wins, else publication.default_action<br/>match on scope, kind, sensitivity, min_confidence, and the trusted-input gates require_evidence, require_source_run, require_source_session for daemon-origin candidates<br/>output action: manual_review, reject, publish_provisional, or publish_active"]
    Judge["JUDGE, optional<br/>model-backed review, runs AFTER rules, CLAMPED to the policy envelope and never widens<br/>in enabled mode a failure fails closed to manual_review, and the verdict is retained for audit"]
    Verification["VERIFICATION, gate<br/>only when the surviving action is publish_active<br/>needs DAEMON-OWNED support, a run-memory record or referenced debug artifact, else DOWNGRADE to publish_provisional<br/>same-subject different-value conflicts escalate to manual_review unless a supersession is named"]
    Publish["PUBLISH<br/>a LearningView is written with status, publish_tier, verification_status, policy_decision, policy_actor<br/>provisional is durable but NOT prompt-visible<br/>active is eligible for retrieval"]
    Inject["INJECT<br/>on a later input, eligible learnings for the visible scopes are packed into ONE learned_context section<br/>only fact, preference, decision, and only if they pass every visibility gate"]
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> VERIFIED : verification run output matches the expected marker, recorded via rollout-result
    VERIFIED --> CANARY : operator records canary evidence
    CANARY --> ACTIVE : at least 1 successful canary and 0 canary failures for the current fingerprint
    ACTIVE --> REVOKED : revoke, or the source learning is revoked
```
