...
```text
                    HOW ONE AGENT IS ASSEMBLED

   ┌──────────────────────────────────────────────────────────────┐
   │  PERSONA RECORD (mutable, daemon-owned)                        │
   │  soul + display_name + version + capability baseline + defaults│
   └───────────────┬──────────────────────────────────────────────┘
                   │  bind at session create (or later)
                   v   -> IMMUTABLE snapshot copied into the session
   ┌──────────────────────────────────────────────────────────────┐
   │  SESSION                                                       │
   │  persona snapshot + session capability_scope + credential_scope│
   │  + tool_overrides + permission mode                            │
   └───────┬───────────────────────────────────┬──────────────────┘
           │                                   │
           v                                   v
   ┌───────────────────────┐         ┌───────────────────────────┐
   │ CAPABILITY SCOPE       │         │ CREDENTIAL SCOPE           │
   │ = persona ∩ session    │         │ = persona/parent ∩ session │
   │ (VISIBILITY)           │         │ (AUTHORIZATION)            │
   │  skill_allow/deny      │         │  route_allow/deny (families)│
   │  mcp_server_allow/deny │         │  connector_allow/deny      │
   │  mcp_tool_allow/deny   │         │  connector_credential_*    │
   │                        │         │  mcp_server_allow/deny     │
   │ decides WHAT IS SEEN   │         │ decides WHAT RESOLVES      │
   └──────────┬─────────────┘         └───────────┬───────────────┘
              │                                   │
              v                                   v
   ┌──────────────────────────────────────────────────────────────┐
   │  EFFECTIVE TOOL SURFACE (per run)                              │
   │  profile tools -> narrowed by capability scope -> session      │
   │  tool_overrides -> operator-contact gating (last word)         │
   └───────────────┬──────────────────────────────────────────────┘
                   │  spawn_agent
                   v
   ┌──────────────────────────────────────────────────────────────┐
   │  CHILD AGENT (sidechain)                                       │
   │  inherits persona snapshot + capability + credential + mode    │
   │  then INTERSECTS with any requested child scope (never wider)  │
   │  optional dedicated git worktree; retain / close_on_settle     │
   └──────────────────────────────────────────────────────────────┘
```
...
```text
   PERSONA: MUTABLE RECORD vs IMMUTABLE BINDING

   PERSONA RECORD  "reviewer"  (daemon state root)
   ┌───────────────────────────────────────┐
   │ version 1  soul="...v1..."             │
   │ version 2  soul="...v2..."  <- current │
   │ capability baseline, default_skills    │
   └───────────────┬───────────────────────┘
                   │
       bind time copies the CURRENT state into the session
                   │
     ┌─────────────┼──────────────────────────────┐
     v             v                              v
  session A     session B                     session C
  bound v1      bound v1                       bound v2
  (frozen)      (frozen)                       (frozen)
     │             │                              │
     └─ updating the record to v3 does NOT rewrite A, B, or C ─┘
        A and B keep v1; C keeps v2; only NEW sessions get v3
```
...
```text
   CAPABILITY vs CREDENTIAL: two independent gates

                 ┌──────────────────────────────┐
   a tool /      │  CAPABILITY SCOPE (visibility)│  passes?
   MCP call ───► │  is it in the allowed surface?│ ──yes──┐
                 └──────────────────────────────┘        │
                                                          v
                 ┌──────────────────────────────┐   ┌──────────┐
                 │  CREDENTIAL SCOPE (authz)     │   │ VISIBLE  │
                 │  does the route/connector/    │   │ but can  │
                 │  secret actually resolve?     │   │ it RUN?  │
                 └───────────┬──────────────────┘   └────┬─────┘
                             │                            │
                    ┌────────┴────────┐                   │
                    v                 v                   │
            ...
            RUNS for real           v                     │
                              tool is VISIBLE but         │
                              the call cannot resolve  ◄──┘
                              its credential -> fails
```
...
```text
   WHERE A VISIBLE SKILL COMES FROM

   CATALOG SKILLS                        PROMOTED PROCEDURAL SKILLS
   ──────────────                        ──────────────────────────
   author writes SKILL.md                a `procedure` learning is
   in a skills/ root                     reviewed and published (active,
        │                                workspace scope, active tier)
        v                                       │
   discovered by the registry                   v  promote (explicit step)
   (Explicit / Repo / User roots,         a draft promoted skill (fork-only,
    scope precedence, digest)             verification profile)
        │                                       │  rollout ladder:
        │                                       v  draft->verified->canary->active
        └──────────────┬──────────────── only ACTIVE promoted skills mount
                        v
             GET /v1/skills  (the runtime catalog)
                        │
                        v
             filtered by the session's EFFECTIVE CAPABILITY SCOPE
                        │
                        v
                visible_skills for the session
```
...
```text
   ...
   1. AGENT PROFILE          default / planning / coordination / verification
      base allow/deny        (the daemon-enforced role surface)
            │
            v
   2. MCP-DERIVED FILTER      MCP servers/tools contribute to the surface
            │
            v
   3. CAPABILITY SCOPE        narrow to what the session may SEE
            │
            v
   4. SESSION TOOL OVERRIDES  enable: un-deny + add to allow
            │                 disable: append to deny
            v
   5. OPERATOR-CONTACT GATING operator tools gated LAST -> overrides here
            │                 cannot bypass operator policy
            v
      final tool surface handed to the model
```
...
```text
   CHILD SCOPE AT SPAWN

   parent effective scope ──┐
                            ├─►  validate: requested ⊆ parent ?
   requested child scope ───┘        │
                                     ├─ no  -> REJECT (cannot widen)
                                     └─ yes -> child = parent.restrict_with(requested)
                                                │
                                                v
                                   allow-lists intersect, deny-lists union
                                   credential defaults-to-none preserved
                                   persisted as the child session scope
```
