...
Terminal tasks (completed, failed, cancelled) also leave the hot control state: each one is appended once to a per-session archive (`sessions/__safe/<name>.tasks-archive.jsonl`), so the control-state snapshot and the system prompt stay bounded by the work in flight. Archived tasks remain visible through `tasks list`, `tasks get`, task output views, and Flow projections, but they are immutable; deleting a live task leaves a tombstone in the same archive so a stale run snapshot can never resurrect it.
