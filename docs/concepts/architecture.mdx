...
```text
                          KHEISH DAEMON — CONTROL, SERVICE, ENGINE, PROVIDER

   external world                     the daemon process                         on disk (state root)
  ┌───────────────┐   ┌──────────────────────────────────────────────────┐   ┌────────────────────────┐
  │ HTTP / SSE    │   │  CONTROL PLANE                                     │   │ sessions/              │
  │ CLI           │──▶│  · request metadata validation                    │   │  __safe/<id>.jsonl      │  journal
  │ ingress       │   │  · route + auth_ref resolution                     │   │  __safe/<id>.meta/*.json │ sidecars
  │ connectors    │   │  · reply-target resolution                         │   │  __safe/<id>.tasks-      │ task
  └───────────────┘   │  · create run  |  queue run  |  resume run         │   │        archive.jsonl      │ archive
                      └───────────────────────┬──────────────────────────┘   ├────────────────────────┤
                                              │ submit / resume               │ runs/__safe/<id>.json   │ run record
                      ┌───────────────────────▼──────────────────────────┐   │ run-events/__safe/*.jsonl│ run events
                      │  SERVICE LAYER                                     │──▶│ run-memories/           │
                      │  run store · session store · scheduler            │   ├────────────────────────┤
                      │  delivery queue · persona/secret stores           │   │ deliveries + dead-letter │
                      │  idempotency ledgers · reply-target index         │   │   ledgers                │
                      └───────────────────────┬──────────────────────────┘   ├────────────────────────┤
                                              │ launch_run(run_id)            │ personas / secrets /     │
                      ┌───────────────────────▼──────────────────────────┐   │   connectors / auth      │
                      │  ENGINE (kheish-core agent loop)                  │   └────────────────────────┘
                      │  prompt build · permissions · tools · compaction  │
                      │  completion boundary · output contract            │
                      └───────────────────────┬──────────────────────────┘
                                              │ ModelRequest
                      ┌───────────────────────▼──────────────────────────┐
                      │  PROVIDER LAYER (model driver + routing)          │
                      │  provider-agnostic · returns answer or typed error│
                      └──────────────────────────────────────────────────┘
```
...
```text
   INPUT → ENVELOPE → ENGINE → DELIVERY

   caller input                    ingress boundary                     run + engine                       output
  ┌──────────────┐   ┌────────────────────────────────┐   ┌──────────────────────────────┐   ┌────────────────────────┐
  │ content /    │   │ · has-payload check             │   │ status: Queued → Running     │   │ output records captured │
  │ input_items  │──▶│ · input-contract validation ────┼──▶│ engine turn loop:            │──▶│ reply targets resolved   │
  │ attachments  │   │   (fail-closed, JSON-path error) │   │  build→model→perms→tools     │   │ output plugins invoked   │
  │ route/model  │   │ · normalize + canonical JSON     │   │ flush journal each boundary  │   │ delivery queue (retry)   │
  │ metadata     │   │ · pin route (override>policy>def)│   │ completion boundary:         │   │  └─ dead-letter on        │
  │ reply targets│   │ · build InputEnvelope           │   │  requirements + out-contract │   │     exhausted budget      │
  └──────────────┘   │ · write run record + events     │   │ → Completed / Waiting / Fail │   └────────────────────────┘
                     └────────────────────────────────┘   └──────────────────────────────┘
        (rejected here                                        (suspends here for                    (canonical JSON to
         never creates a run)                                  approvals / questions)                webhooks; see below)
```
...
```text
   THE RUN STATE MACHINE  (arrows = legal transitions; terminal states absorb)

                         submit / promote
        ┌─────────┐  ────────────────────────▶  ┌──────────┐
        │ QUEUED  │                              │ RUNNING  │◀───────────────┐
        └────┬────┘                              └────┬─────┘                │
             │  (reject before start)                 │                      │  resume
             │  ├──────────────▶ FAILED               │                      │  (approval
             │  └──────────────▶ CANCELLED            │                      │   resolved)
             │                                        │                      │
             │        ┌───────────────────────────────┼──────────────┐      │
             │        │            │            │      │       │       │      │
             ▼        ▼            ▼            ▼      ▼       ▼       ▼      │
        (queued  WAITING_FOR   WAITING_FOR  COMPLETED FAILED INTERR. CANCEL  │
         cannot   APPROVAL      USER_QUESTION   ▲       ▲      ▲       ▲      │
         reach    │  │  │  │     │  │  │  │  │   │       │      │       │      │
         waiting  │  │  │  │     │  │  │  │  └───┘       │      │       │      │
         states)  │  │  │  └──▶ RUNNING ───────┐        │      │       │      │
                  │  │  │        ▲             │  resume │      │       │      │
   ...
   Legal transitions, exhaustively:
     ...
     RUNNING                → WAITING_FOR_APPROVAL · WAITING_FOR_USER_QUESTION
     ...
     COMPLETED / FAILED / INTERRUPTED / CANCELLED → (nothing; terminal)
```
...
```text
   SESSION QUEUE  (one active run; the rest wait)

   session S ─┬─ active_run_id ──▶ [ run-42  RUNNING ]
              │
              └─ queued_run_ids ──▶ [ run-43 QUEUED ] [ run-44 QUEUED ] [ run-45 QUEUED ]

   run-42 reaches a terminal status
        │
        ▼
   promote head of queue
              ┌─ active_run_id ──▶ [ run-43  RUNNING ]
   session S ─┤
              └─ queued_run_ids ──▶ [ run-44 QUEUED ] [ run-45 QUEUED ]
```
...
```text
   IDEMPOTENT SUBMIT  (key + fingerprint)

   submit(key K, payload P)
        │
        ▼
   receipt for K exists? ──yes──▶ fingerprint(P) == stored? ──yes──▶ return existing run
        │ no                                   │ no
        ▼                                      ▼
   acquire in-flight lock for K          409 idempotency_conflict
        │                                (key reused with a different payload)
        ├─ held by another submit ─▶ wait ≤ 2s, poll, then return the run it created
        ▼
   reserve run id · create run · remember receipt(K → run, fingerprint) · release lock
```
...
```text
   ...
   ┌───────────────────────────── session: root ─────────────────────────────┐
   │  agent: root  (persona snapshot · capability scope · credential scope)   │
   │     journal · checkpoints · metadata sidecars · control state            │
   └───────┬─────────────────────────────────────────────────┬───────────────┘
           │ spawns sidechain                                 │ mailbox
           ▼                                                  ▼
   ┌─────────────── session: child-A ───────────┐     [ msg ][ msg ][ msg ]
   │  agent: child-A   parent = root            │            │
   │  inherits persona + capability + credential │           ▼  delivered as
   │  boundaries, may only narrow them           │      mailbox_delivery run
   │                                             │
   │  needs a decision from its parent           │
   │        │                                    │
   │        └── raises parent clarification ──────┼────────────▶ parent_clarification
   └─────────────────────────────────────────────┘              run on session: root
```
...
```text
   TURN-BOUNDARY TIMELINE  (durability boundaries marked ▣)

   run starts
     │
     ├─▣ flush: accepted input becomes durable
     │
     ├─ TURN n ─────────────────────────────────────────────┐
     │    build prompt · (maybe compact) · call model        │
     │    append assistant message + ToolCallStarted entries │
     │  ▣ flush: assistant + tool-calls durable BEFORE tools │
     │    execute tools                                       │
     │    append ToolCallFinished entries                    │
     │  ▣ flush: tool results durable BEFORE next model call │
     │                                                        │
     ├─ TURN n+1 ──────────────────────────────────────────┐ │
     │  ▣ flush (top of turn) · … repeats …                 │ │
     │                                                       ─┘
     │
     └─ completion boundary reached
          │
          └─ end-of-run batched persist (dedup against tail → append new suffix only)
```
...
```text
   VACUUM SAFETY CONTRACT

   preconditions:  state-root lock held  ·  no existing <name>.jsonl.vacuum-bak
        │
        ▼
   pass 1: index the last line holding each metadata key
challenge   pass 2: stream rewrite → temp file
        │            keep every non-metadata record in original order
        │            re-emit only the surviving metadata records (last per key)
        ▼
   verify: stream_summary(original) == stream_summary(temp) ?
        │                                   │
        │ no ─────────────────────────────▶ abort · remove temp · ORIGINAL UNTOUCHED
        │ yes
        ▼
   hard-link original → <name>.jsonl.vacuum-bak     (backup preserved first)
   atomic rename temp → <name>.jsonl                (swap only after backup + verify)
   fsync parent dir
        │
        ▼
   report: bytes_before / bytes_after · metadata_records_dropped · torn_tail_dropped
```
