...
- `board_draw`: append vector elements (paths, lines, arrows, rects, ellipses, text) to a shared whiteboard as a new CAS-guarded revision; the daemon rasterizes them over the previous render and stamps each batch with the agent's name and stable per-session color
