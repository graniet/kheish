...
Agents draw with the `board_draw` control tool: the daemon appends the batch to the latest state, rasterizes it over the previous render, stamps every element with the agent's name and stable color, and paints a small author tag next to the batch. Concurrent writers retry against the fresh tip a bounded number of times, so a human saving from the console and an agent drawing from a run can safely alternate on the same board.
