...
```mermaid
flowchart TD
    Daemon["kheish-daemon<br/>control plane, agent loop, workers"]
    Daemon -->|"writes durable state"| State["STATE ROOT<br/>back this up<br/>it is the memory of the daemon"]
    Daemon -->|"runs shell and file tools"| Workspace["WORKSPACE ROOT<br/>blast radius<br/>ephemeral by design"]
```
...
```mermaid
flowchart TD
    Start["kheish-daemon serve or up, --state-root R"]
    Start --> S1["1. Acquire the state-root lock, flock on daemon.lock.<br/>If another daemon holds it, refuse.<br/>Same mechanism as sessions vacuum, so vacuum-while-serving fails politely."]
    S1 --> S2["2. Load runtime-config.json, restore the LAST revision.<br/>route or model, permission mode, hooks, debug level, tool limits, learning and run-memory policy.<br/>BEFORE accepting work."]
    S2 --> S3["3. Rebuild the route registry.<br/>routes file, path and SHA from route-inventory.json.<br/>plus routes-overlay.json, insertion-ordered, first stays default if the file inventory was empty.<br/>plus resolve auth_ref slots from the encrypted auth store."]
    S3 --> S4["4. Replay durable stores: sessions journal and sidecars, schedules, channels records leases stimuli thread-work, projects, personas, connectors, deliveries, learnings."]
    S4 --> S5["5. Repair run lifecycle.<br/>one wait-capable run per session keeps the active slot.<br/>extra daemon-owned replayable runs, scheduled, mailbox, observation, parent-clarification, are requeued.<br/>extra normal input runs are INTERRUPTED, not replayed.<br/>active shell tasks are SETTLED FAIL-CLOSED: status=failed, terminal_reason=daemon_restarted, recovered_on_boot=true."]
    S5 --> S6["6. Start workers, scheduler, delivery, learning, open the HTTP listener, /readyz flips to 200."]
```
...
```mermaid
flowchart TD
    V1["1. SLO warning fires: oversized_session_count above 0"]
    V1 --> V2["2. Confirm the target from /v1/status .storage.session_storage using largest_session_id and oversized_session_ids"]
    V2 --> V3["3. Drain and STOP the daemon for that state root, see Upgrades"]
    V3 --> V4["4. kheish-daemon sessions vacuum the session id --state-root R"]
    V4 --> V5["5. Verify the JSON report: bytes_after much less than bytes_before, metadata_records_dropped above 0, torn_tail_dropped as expected"]
    V5 --> V6["6. Restart the daemon. Confirm /readyz=200 and runs start normally"]
    V6 --> V7["7. Remove the .jsonl.vacuum-bak backup once you trust the compaction"]
```
...
flowchart TD
    subgraph Host["host"]
        CLI["operator CLI or SDK"]
        Daemon["kheish-daemon"]
        State["state_root"]
        Workspace["workspace_root"]
        CLI -->|"127.0.0.1:4000"| Daemon
        Daemon --> State
        Daemon --> Workspace
    end
```
...
flowchart LR
    Net["Internet or VPN"]
    Proxy["Nginx or Caddy<br/>TLS terminate<br/>SSE buffering OFF"]
    Daemon["kheish-daemon<br/>127.0.0.1:4000<br/>bearer enabled"]
    Orch["orchestrator"]
    Net -->|"https to kheish.example.com"| Proxy
    Proxy -->|"plain HTTP loopback, Authorization forwarded, X-Forwarded headers set"| Daemon
    Orch -->|"/healthz and /readyz reachable, SSE streams NOT buffered"| Proxy
```
...
flowchart LR
    subgraph Orch["orchestrator"]
        Daemon["kheish-daemon<br/>0.0.0.0:4000<br/>bearer auth<br/>MCP discovery OFF"]
        Connector["connector service<br/>webhook, slack, telegram"]
        State["state volume<br/>/var/lib/kheish/state"]
        Work["workspace volume<br/>/workspace"]
        Secrets["secrets<br/>/run/secrets/... Docker-managed, 0600 on host"]
        Daemon <-->|"remote_http /v1/connectors/external"| Connector
        Daemon --> State
        Daemon --> Work
        Daemon --> Secrets
    end
...
```mermaid
flowchart TD
    Map["Observability surfaces"]
    Map --> Unauth["Unauthenticated probes<br/>outside auth middleware"]
    Map --> Snapshot["Authenticated operator snapshot"]
    Map --> Ring["Daemon log ring buffer"]
    Map --> Delivery["Delivery and DLQ"]
    Map --> Streams["Live streams SSE"]
    Map --> Audit["Security audit on disk"]
```
...
flowchart TD
    T1["t0 SIGTERM sent"]
    T2["t0 drain begins, /readyz returns 503"]
    T3["ingress tasks and MCP shutdown run"]
    T4["t+ listening PORT released<br/>orchestrator may wrongly start a new daemon here"]
    T5["t++ process exits, flock on daemon.lock RELEASED<br/>only now is start safe"]
    Gap["gap up to about 2s between port release and lock release<br/>do NOT race into it"]
    T1 --> T2 --> T3 --> T4 --> T5
    T4 -.-> Gap
    T5 -.-> Gap
```
...
```mermaid
flowchart TD
    U1["1. Build or pull the new binary or image. Record the version or digest."]
    U1 --> U2["2. Snapshot or archive the state root, see Backup. Optionally the workspace."]
    U2 --> U3["3. Drain: stop accepting new external ingress if you can. Let active runs settle, or accept that active shell tasks will be settled fail-closed."]
    U3 --> U4["4. Stop the old daemon. CONFIRM the process exited and the flock released."]
    U4 --> U5["5. Start the new daemon on the SAME state root, same routes file, same auth-store master key."]
    U5 --> U6["6. Validate: /readyz=200, status.health.ok, runtime get, doctor, doctor routes --check-auth, doctor routes --check-references."]
    U6 --> U7["7. Watch /v1/status warnings and /v1/logs for the first few minutes."]
```
...
```mermaid
flowchart TD
    Q["/readyz stuck on 503, not draining?"]
    Q --> C1["Another daemon holds the state-root lock, flock?"]
    C1 --> R1["Check for a lingering old process, wait out the about 2s flock gap"]
    Q --> C2["runtime-config.json unreadable or route rebuild failing?"]
    C2 --> R2["doctor routes --check-auth, confirm auth-store master key present"]
    Q --> C3["Storage write-probe failing?"]
    C3 --> R3["status.storage.probes, check disk full, permissions, inode"]
    Q --> C4["Provider route not ready?"]
    C4 --> R4["status.provider_readiness, doctor routes --check-references"]
```
...
```mermaid
flowchart TD
    Q["status.runs.queued climbing, oldest_queued_run_age_ms large?"]
    Q --> C1["Active run stuck waiting for approval or question?"]
    C1 --> R1["status.runs.waiting_for_*, approvals list, questions list"]
    Q --> C2["Oversized session slowing run start, old build symptom?"]
    C2 --> R2["status.storage.session_storage, plan a vacuum window"]
    Q --> C3["Provider route degraded?"]
    C3 --> R3["status.provider_readiness, doctor routes --canary"]
    Q --> C4["A stale non-terminal run holding the slot?"]
    C4 --> R4["status.runs.oldest_non_terminal_run_idle_ms, runs get the run id, runs cancel it if genuinely stuck"]
```
...
```mermaid
flowchart TD
    Q["Replies not arriving?"]
    Q --> C1["status.delivery.unresolved_dead_lettered above 0?"]
    C1 --> R1["deliveries dead-letter, inspect safe error codes, fix target, deliveries replay the id"]
    Q --> C2["Wrong reply target on the run?"]
    C2 --> R2["runs get the run_id, reply targets snapshot at run or task creation<br/>a session reply-target edit is prospective, not retroactive"]
    Q --> C3["Connector broken?"]
    C3 --> R3["connectors get kind and name, runtime connectors external metrics"]
```
