...
```
  ...
  model emits          ┌─────────────────────────────────────────────┐
  tool_call  ─────────▶│ 1. resolve tool by name                     │
  {name,input}         │    (built-in? control tool? mcp__srv__tool?) │
                       └───────────────┬─────────────────────────────┘
                                       │ unknown name → hard error
                                       ▼
                       ┌─────────────────────────────────────────────┐
                       │ 2. ToolRuntimeLimits gate                    │
                       │    input bytes, calls-per-turn, batch width  │
                       └───────────────┬─────────────────────────────┘
                                       │ over limit → rejected, audited
                                       ▼
                       ┌─────────────────────────────────────────────┐
                       │ 3. capability scope: is this tool visible?   │
                       │    (persona / session / sidechain boundary)  │
                       └───────────────┬─────────────────────────────┘
                                       │ hidden → not callable at all
                                       ▼
                       ┌─────────────────────────────────────────────┐
                       │ 4. permission engine                         │
                       │    rule match → mode transform → decision    │
                       └───────┬──────────────┬──────────────┬────────┘
                          allow│           ask│           deny│
                               ▼              ▼               ▼
                       ┌───────────┐  ┌──────────────┐  ┌───────────┐
                       │ 5a. run   │  │ 5b. persist  │  │ 5c. deny  │
                       │ the tool  │  │ approval req,│  │ hooks,    │
                       │           │  │ pause run,   │  │ audit,    │
                       │           │  │ wait for a   │  │ return    │
                       │           │  │ human, then  │  │ refusal   │
                       │           │  │ resume       │  │           │
                       └─────┬─────┘  └──────┬───────┘  └─────┬─────┘
                             ...
                             (for external effects) signed
                             external-action ledger record
```
...
```
apply_patch            get_goal               schedule_trigger_now
ask_operator           glob_search            set_channel_reaction
ask_user_question      grep_search            spawn_agent
bash                   list_agent_summaries   store_asset
create_channel_stimulus list_agents           task_create
create_goal            list_files             task_delete
edit_file              list_skills            task_get
edit_image             message_agent          task_list
emit_output            notify_operator        task_output
enter_plan_mode        read_channel_thread    task_stop
exit_plan_mode         read_file              task_update
generate_audio         request_parent_        todo_write
generate_image           clarification        update_goal
get_agent              schedule_cancel        use_skill
                       schedule_create        wait_agent
                       schedule_get           wake_after
                       schedule_list          wake_at
                       schedule_pause         web_fetch
                       schedule_resume        web_search
                       schedule_resume        write_file
```
...
```
        RULE RESOLUTION: SCOPE  →  SPECIFICITY  →  MODE

   ┌──────────────────────────────────────────────────────┐
   │ durable session hook updates merged with static       │
   │ session rules (hook updates win same-pattern ties)    │
   └───────────────────────┬──────────────────────────────┘
                           ▼
   ┌──────────────────────────────────────────────────────┐
   │ scope order:   session  ▶  project  ▶  user           │
   └───────────────────────┬──────────────────────────────┘
                           ▼
   ┌──────────────────────────────────────────────────────┐
   │ within the winning scope, most specific pattern wins: │
   │   exact name  ▶  longer prefix (mcp__linear__*)       │
   │              ▶  shorter prefix (mcp__*)  ▶  *          │
   │   ties broken by earliest-inserted rule               │
   └───────────────────────┬──────────────────────────────┘
                           ▼
   ┌──────────────────────────────────────────────────────┐
   │ apply effective mode: ask→allow, ask→deny,            │
   │ or plan-deny a base allow for non-whitelisted tools   │
   └───────────────────────┬──────────────────────────────┘
              decision = allow / ask / deny
                           ▼
   ┌──────────────────────────────────────────────────────┐
   │ if ask → run permission_request hooks                 │
   │ if deny → run permission_denied hooks                 │
   │ persist the permission audit for this call            │
   └──────────────────────────────────────────────────────┘
```
...
```
   AGENT                         KHEISH AIR CHAT                 HUMAN
     │                                 │                           │
     │ tool_call: bash "rm -rf build"  │                           │
     │────────────────────────────────▶│                           │
     │                                 │ ┌───────────────────────┐ │
     │        run is SUSPENDED         │ │  ⚠ Approval Needed    │ │
     │        (nothing executes)       │ │  bash                 │ │
     │                                 │ │  $ rm -rf build       │ │
     │                                 │ │  [ Approve ] [ Deny ] │ │
     │                                 │ └──────────┬────────────┘ │
     │                                 │            │  click Approve│
...
```
   ...
   │                                                                 │
   │   ┌───────────────┐   loads once at boot   ┌────────────────┐   │
   │   │  MCP Manager  │◀──────────────────────▶│  Auth store    │   │
   │   │  (global      │   resolves mcp.* slots  │  (encrypted    │   │
   │   │   inventory)  │                         │   secrets)     │   │
   │   └───┬───────┬───┘                         └────────────────┘   │
   │       │       │                                                  │
   ...
   │  ┌─────────┐ ┌────────────────────────────────┐                 │
   │  │ child   │ │ HTTP client                     │                 │
   │  │ process │ │  auth: None | Bearer | OAuth    │                 │
   │  │ (stdin/ │ │  bearer/header from secret_ref  │                 │
   │  │  stdout │ │  OAuth re-authorizes each call  │                 │
   │  │  JSON-  │ └──────────────┬─────────────────┘                  │
   │  │  RPC)   │                │                                    │
   │  │ env from│                │ https                              │
   │  │ secret_ │                ▼                                    │
   │  │ refs,   │      ┌──────────────────────┐                       │
   │  │ restricted     │  remote MCP server   │                       │
   │  │ env     │      │  (Linear, Stripe,    │                       │
   │  └────┬────┘      │   your internal MCP) │                       │
   │       │           └──────────────────────┘                      │
   │       ▼                                                          │
   │  ┌──────────────────────┐                                       │
   │  │ local MCP server      │  e.g. GitHub via Docker,              │
   │  │ (Docker / npx / bin)  │  Playwright, Sentry                   │
   │  └──────────────────────┘                                       │
   │                                                                 │
   │   surfaced into runtime as:  mcp__<server>__<tool>              │
   │   plus helpers: list_mcp_resources, list_mcp_resource_          │
   │                 templates, read_mcp_resource                    │
   └─────────────────────────────────────────────────────────────────┘
```
...
```
        WHAT AN MCP TOOL CALL MUST PASS

   global daemon MCP inventory
        │   (every loaded server + tool)
        ▼
   ┌─────────────────────────────┐
   │ CapabilityScope             │   hides whole servers,
   │  visible? ──── no ──▶ NOT   │   individual tools, and
   │        │             CALL-  │   resource helpers
   │       yes            ABLE   │
   └────────┼────────────────────┘
            ▼
   ┌─────────────────────────────┐
   │ Permission engine           │   mcp__* defaults to ask;
   │  allow / ask / deny         │   deny rules still win
   └────────┼────────────────────┘
            ▼ (allow or approved)
   ┌─────────────────────────────┐
   │ CredentialScope             │   even if visible + allowed,
   │  mcp_server_allow / deny    │   a credentialed server can
   │  usable? ── no ──▶ DENIED   │   be denied at resolution
   │        │          AT USE    │
   │       yes                   │
   └────────┼────────────────────┘
            ▼
...
```
        ONE MODEL TURN → MANY TOOL CALLS

   model returns a batch of N tool calls in one turn
        │
        ▼
   is N ≤ max_calls_per_turn (256)? ── no ──▶ batch rejected
        │ yes
        ▼
   split into chunks of max_parallel_tools (16)

   chunk 1                      chunk 2
   ┌────┬────┬────┬────┐        ┌────┬────┬────┐
   │ t1 │ t2 │ t3 │ .. │  then  │ t17│ t18│ .. │  ...
   └─┬──┴─┬──┴─┬──┴────┘        └────┴────┴────┘
     │    │    │  each ≤ its own timeout_ms (≤ max_timeout_ms)
     ▼    ▼    ▼  each ≤ max_output_bytes
   results accumulate; if cumulative output or envelope bytes
   exceed the batch caps, further calls in the turn are cut off
        │
        ▼
   a tool with allows_parallel=false runs alone, not in a chunk
```
