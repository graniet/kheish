...
```mermaid
flowchart LR
    C["CONNECTOR, a transport definition<br/>bot token or signing key<br/>webhook vs polling<br/>rate limits and allow-lists<br/>stable connector name<br/>owned by the DAEMON"]
    RT["REPLY TARGET, a routing destination<br/>plugin such as telegram<br/>address, an opaque route<br/>lives on a session, run, or connector"]
    C -->|"derives a natural reply target, e.g. reply in the same chat or thread"| RT
```
...
```mermaid
flowchart LR
    subgraph WH["WEBHOOK, the default"]
        TG1["Telegram"] -->|"HTTPS route, verified by secret_token header"| D1["daemon"]
    end
    subgraph PO["POLLING"]
        D2["daemon"] -->|"getUpdates long-poll, timeout is polling_timeout_seconds"| TG2["Telegram"]
    end
```
...
```mermaid
flowchart TD
    E["1. EXPLICIT output override, this specific emit says go here"] -->|"none"| RS["2. RUN SNAPSHOT, targets captured when the run was created, immutable"]
    RS -->|"none"| SD["3. SESSION DEFAULTS, durable session reply-target preferences, editable"]
```

A run captures its reply targets at creation time. Editing the SESSION defaults afterward does NOT rewrite that run's snapshot. It only affects FUTURE work and future fallback.
...
```mermaid
flowchart TD
    A["Inbound event, connector A, reply target X"] --> RA["Run created, snapshot reply targets X"]
    RA --> DA["Delivered to X"]
    RA --> SA["Session default reply targets set to X, written back"]
    B["Later, connector B sends an event with target Y"] --> RB["Run created, snapshot Y"]
    RB --> DB["Delivered to Y"]
    RB --> SB["Session default reply targets set to Y, overwrites X"]
```

Net effect: whichever connector arrived LAST sets the session's default output route. Daemon-owned output that falls back to session defaults follows the last arrival.
...
```mermaid
flowchart TD
    CH{"Choose external connector run mode"}
    CH -->|"you operate it elsewhere"| RH["remote_http<br/>sidecar runs somewhere else, you operate it<br/>base_url public or private<br/>private nets need allow_private_network true<br/>you manage lifecycle, restarts, and secrets<br/>good for shared infra or a sidecar behind your own load balancer"]
    CH -->|"turnkey single host"| CP["child_process<br/>daemon launches and supervises the sidecar as a subprocess<br/>base_url loopback only, such as 127.0.0.1 or localhost, no path, query, or fragment<br/>daemon manages lifecycle, restarts, and secret leasing<br/>good for single-host deploys and turnkey setups"]
```
...
sequenceDiagram
    ...
    D->>S: spawn process with reserved env vars, listed below
    Note over S: process starts
    D->>S: GET /manifest, daemon polls readiness
    S-->>D: protocol_version 1 and instance_id
    D->>S: GET /health
    S-->>D: SAME instance_id, readiness gate passes
    S->>D: GET /v1/connectors/external/NAME/credentials/DISCORD_BOT_TOKEN, Bearer credential token
    D-->>S: value, lease_id, grant_id, expires_at_ms
    S->>P: sidecar connects to Discord and marks ready
    P->>S: inbound event arrives, sidecar builds a Kheish event
    S->>D: POST /v1/connectors/external/NAME/events, ingress protocol 2
    D-->>S: status, session_id, run_id
    Note over D: run produces output
    D->>S: POST /deliver protocol 1, Idempotency-Key kheish delivery_id
    S->>P: sidecar sends to the platform
    S-->>D: 2xx committed
```

Reserved environment variables the daemon sets on spawn:
...
```mermaid
flowchart TD
    CFG["configure"] --> VAL["validate secret slots"]
    VAL --> SPAWN["spawn in its own process group"]
    SPAWN --> POLL["poll /manifest and /health"]
    POLL --> CHK{"same instance_id?"}
    CHK -->|"yes"| READY["READY, serve ingress and delivery"]
    CHK -->|"no or timeout"| RETRY["retry with backoff"]
    RETRY --> POLL
    READY --> SHUT["daemon shutdown, signal the process group"]
    SHUT --> EXIT["child exits"]
    EXIT --> REVOKE["credential lease revoked"]
```
...
```mermaid
flowchart TD
    S1["1. connector fixed_session_id, everything into ONE session"] -->|"not set"| S2["2. a session already bound to the event's derived binding keys, sticky binding keys"]
    S2 -->|"none"| S3["3. natural session id from thread.path, hierarchical conversation identity of guild, channel, thread"]
    S3 -->|"thread.path empty"| S4["4. natural session id from routing_key, threadless domain events"]
    S4 -->|"none of the above"| REJ["REJECTED"]
```

Rule: if thread.path is present, it drives identity. routing_key is only used for identity when thread.path is empty, but routing_key is always PERSISTED to metadata.
...
Tiers from most to least production-ready:

```mermaid
flowchart TD
    PROD["production: discord, email, sms, webhook<br/>native transports are telegram, slack, http"] --> BE["best-effort: matrix<br/>unencrypted rooms only, no E2E crypto"]
    BE --> EXP["experimental: signal, whatsapp<br/>depend on external services you run, not SLA-ready"]
```
