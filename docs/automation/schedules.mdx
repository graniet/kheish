...
```text
   ...
   "run this at   ->  once      { fire_at_ms }
    a specific
    moment"

   "run this      ->  interval  { every_seconds >= 5 }
    every N
    seconds"

   "run this on   ->  cron      { expression, timezone }
    a calendar
    pattern"

   Exactly ONE cadence per schedule. There is no
   hybrid. If you need "every weekday at 9 AND on the
   1st of the month", that is TWO cron schedules, not
...
```text
   DST BEHAVIOR (Europe/Paris, expression "0 30 2 * * *")
   ======================================================

   Spring forward (March):        Fall back (October):
   01:59  02:00 ──jump──> 03:00   01:59  02:30(#1)  02:00-again  02:30(#2)
          └ 02:30 never exists           └────── coalesced to ONE fire ─┘
          -> fires next valid day        -> fires exactly once
```
...
```text
   FIRE PIPELINE:  due  ->  overlap check  ->  outcome
   ===================================================

                    a fire becomes DUE
                           │
                           ▼
                 is a prior execution
                    still in flight?
                     /           \
                   NO             YES
                    │              │
              DISPATCH        ┌────┴─────────────┐
                              │ overlap_policy?  │
                              ├──────────────────┤
                   skip  ───► drop this fire, advance next_fire
                   queue_one► remember ONE fire, run it on settle
                   parallel ► dispatch now, run alongside
                              └──────────────────┘

   max_executions is checked BEFORE dispatch. If the
   schedule has no remaining execution slots, even a
   queued fire is skipped with a recorded reason.
```
...
```text
   MISFIRE RECONCILIATION (per-minute interval, 50 min outage)
   ===========================================================

   fires that SHOULD have happened while daemon was down:
   09:01 09:02 09:03 ... 09:49 09:50   (50 missed)

   coalesce_once ─► [ one catch-up run ] ─► resume at 09:51, 09:52 ...
   skip_missed   ─► ( nothing )          ─► resume at 09:51, 09:52 ...

   Both converge on the same FUTURE schedule. They differ
   only in whether the gap produces one run or zero runs.
```
...
```text
   ...
   COMPLETED  reached max_executions, or a `once` fired
   CANCELED   operator/agent stopped it permanently

   COMPLETED and CANCELED are TERMINAL. You do not
   resume them; you create a new schedule.
```
...
```text
   PAUSE / RESUME  (interval, 12 min remaining at pause)
   =====================================================

   ...──fire──[ 48 min elapsed ]──PAUSE  (remaining = 12m)
                                    :
                          (paused for any duration)
                                    :
                                  RESUME ──[ 12 min ]──► next fire

   Cron is different: resume rejoins the calendar pattern
   from "now"; it does not replay the paused window.
```
...
```text
   ...
   ======================
              ...
              └───────────┘   resume    └────────┘
                 │      │
     budget hit  │      │  mark complete
                 ▼      ▼
        ┌──────────────┐   ┌──────────┐
        │ BUDGET_LIMITED│   │ COMPLETE │
        └──────────────┘   └──────────┘
              │
              │ ONE budget wrap-up continuation, then stop
              ▼
        (no further auto-continuation)

   ACTIVE          daemon drives it when the session is idle
   PAUSED          user/system halted auto-continuation
   BUDGET_LIMITED  hit the token ceiling; allowed ONE wrap-up
