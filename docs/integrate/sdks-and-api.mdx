...
```text
        your code                         the durable engine
   ┌──────────────────┐   HTTPS / HTTP   ┌───────────────────────────┐
   │  Kheish SDK       │  Bearer token   │      kheish-daemon         │
   │  (py/ts/go/rust)  │ ───────────────►│   /v1/* control plane      │
   │                   │                 │                            │
   │  from_env()       │  snake_case     │   sessions · runs ·        │
   │  responses.create │  JSON wire      │   schedules · learnings ·  │
   │  run.wait()       │◄─────────────── │   runtime · connectors ·   │
   │  sessions.create  │  JSON / SSE     │   assets · docs · logs     │
   └──────────────────┘                 └────────────┬──────────────┘
                                                      │
                                        durable state (see production.mdx):
                                        sessions/, runs, deliveries, learnings
```
...
```text
   your code                    SDK                         daemon
      │  responses.create(...)   │                            │
      │ ───────────────────────► │  POST session input / run  │
      │                          │ ─────────────────────────► │  run: queued
      │                          │ ◄───────────────────────── │  202 + run handle
      │  run.wait(...)           │                            │
      │ ───────────────────────► │  poll GET /v1/runs/{id}    │
...
```text
   run.wait_until_blocked(timeout)
        │
        ▼
   ┌──────────────────────────────────────────────┐
   │ blocked.requires_approval ?                    │
   │    yes → blocked.approve_all("ok, operator")   │──┐
   │ blocked.requires_question ?                    │  │
   │    yes → gather answers → blocked.answer(...)  │──┤
   └──────────────────────────────────────────────┘  │
        │                                              │
        ▼                                              ▼
   run resumes ───────────────► run.wait(raise_on_failure=True)
                                     │
                                     ▼
                                output_text
```
...
```text
    ...
    (approval/       │         ▼               ▼           │
     question)      │  ┌───────────────┐ ┌──────────────────┐
                    └──┤ waiting_for_  │ │ waiting_for_user_ │
                       │ approval      │ │ question          │
                       └───────────────┘ └──────────────────┘
                              │
        terminal states (run.wait returns / raises here):
                              ▼
       ┌───────────┬──────────┬─────────────┬────────────┐
...
```text
   SET THE CONTRACT (schema PUT, once per session):
   ...
   THEN, PER RUN — canonical JSON both ways:

   ┌────────────┐   input JSON that      ┌──────────┐   output JSON that
   │ integrator │   MUST match the ─────►│  daemon  │──► MUST match the ───► integrator
   │            │   input schema         │          │   output schema
   └────────────┘   (else 400)          └──────────┘   (daemon repairs up to
                                                        max_repair_attempts)
```
...
```text
   session has reply_target { http, https://your-app/webhook }
        │
        ▼
   run completes / emits output
        │
        ▼
   daemon delivery worker  ──POST──►  https://your-app/webhook
        │   JSON body:                       │
        │   { session_id, thread_id,         │  200 → delivered
        │     content, parts,                │  429 → retry with backoff
        │     resolved_parts, artifacts,     │  fail after retries →
        │     artifact_attachments, metadata}│    dead-letter (replayable)
        ▼
   delivery is durable + retried; DLQ + replay if the target keeps failing
```
...
```text
   submit run  (idempotency_key = "order-8842")
        │
        ▼
   daemon: have I seen "order-8842" for this session?
        ├─ no  → create a new run, remember the key
        └─ yes → return the EXISTING run, do not run again
```
...
```text
   external platform            your sidecar (ExternalDaemonClient)      daemon
        │  event                       │                                   │
        │ ────────────────────────────►│  POST /v1/connectors/external/    │
        │                              │       {name}/events               │
        │                              │  (ingress protocol_version 2,     │
        │                              │   falls back to 1 for old daemons)│
        │                              │ ─────────────────────────────────►│
        │                              │            materializes a session │
        │                              │            + a run, replies via   │
        │                              │            the connector's        │
        │                              │            reply targets          │
```
...
```text
   /v1  control plane (behind auth middleware)
    │
    ├── sessions ─── durable containers: create, input, route policy,
    │                capability/credential scope, reply targets, persona
    │                binding, input/output contracts, memory context/search
    ├── runs ─────── execution attempts: list, get (source of truth for
    │                route+model used), cancel, debug bundles, external actions
    ├── questions & approvals ── the human-loop control plane
    ├── tasks ─────── background daemon tasks: list, output, stop
    ├── schedules ─── durable schedules + wakeups: create, trigger, pause,
    │                resume, cancel
    ├── learnings ─── learning-candidates, learnings, revoke-matching,
    │  & memory       learning-skills (promoted procedural skills)
    ├── runtime ───── /v1/status, /v1/capabilities, /v1/runtime,
    │                learning-policy, secrets, auth subjects/leases, hooks,
    │                debug-level, connectors/external/metrics, routes overlay
    ├── connectors ── runtime connectors + ingress definitions (http/slack/
    │                telegram/external)
    ...
    ├── channels ──── public conversations, members, messages, leases,
    │                stimuli, thread-work
    ├── projects ──── projects, members, linked channels, project tasks
    ├── playbooks &── playbook lifecycle + Flow execution/streaming
    │  flows
    ├── observations ── observation sources, observations, materializations
    │  & derivations   and durable derivations
   ...
   separately mounted (NOT control-plane auth):
    ├── connector ingress ── /v1/connectors/{http,slack,telegram}/{name},
    │                        /v1/connectors/external/{name}/events[/batch],
    │                        /v1/connectors/external/{name}/credentials/{key}
    └── observation upload ── /v1/observation-sources/{id}/observations
                              (source-scoped bearer token, not the admin token)
```
...
```text
   [ Use via SDK ]  on session "orders"
        │
        ▼
   pick a language:  ● Python   ○ TypeScript   ○ Go   ○ Rust
        │
        ├─ reads your connection settings → fills KHEISH_BASE_URL
        ├─ fetches GET /v1/sessions/orders/input-contract
        │     └─ if a contract exists: builds a payload skeleton from the
        │        JSON Schema so the snippet passes validation
        └─ emits the snippet, mirrored from the official kheish-sdks quickstarts
        ▼
   copy → paste → run.  target = this exact session.
```
