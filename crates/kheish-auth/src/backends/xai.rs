...
/// Resolves xAI API-key auth material stored in one daemon-managed auth slot.
pub struct XAiAuthBackend;

impl XAiAuthBackend {
    /// Creates one xAI auth backend.
    pub fn new() -> Self {
        Self
    }

    /// Builds one static xAI API-key slot record.
    pub fn static_api_key_record(
        slot_id: AuthSlotId,
        api_key: impl Into<String>,
    ) -> Result<AuthSlotRecord> {
        let state = XAiStoredState::ApiKey {
    ...
    fn parse_state(record: &AuthSlotRecord) -> Result<XAiStoredState> {
        serde_json::from_value(record.state.clone()).context("invalid stored xAI auth state")
    }
}

#[async_trait]
impl AuthBackend for XAiAuthBackend {
    ...
    async fn resolve(
        ...
        let XAiStoredState::ApiKey { api_key } = Self::parse_state(record)?;
